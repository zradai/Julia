function simC(mortalities, p, N, Y)
  ...
  for y in 1:Y
    ...
    if s==1 # good year
      for n in 1:N
        ...
        if female_strats[n]==1 # check female strategy: 1 = strat 'A' (static bet-hedging)
        ...
        else # female strategy: 0 = strat 'B' (plastic bet-hedging)
          t2_slow = rbinom(1, Int64(floor( (1-dn)*f0 )), (1-mTot_slow_good))[1]
          t1 = rbinom(1, Int64(floor( dn*f0 )), (1-m1_rapid_good))[1]
          t2_rapid = rbinom(1, Int64(floor( t1*f_rapid )), (1-m2_slow_good))[1]
      ...
      for n in 1:N
        ...
        if female_strats[n]==1
          ...
          t2_slow = rbinom(1, Int64(floor( (1-dn)*f0 )), (1-mTot_slow_bad))[1]
          t1 = rbinom(1, Int64(floor( dn*f0 )), (1-m1_rapid_bad))[1]
          t2_rapid = rbinom(1, Int64(floor( t1*f_rapid )), (1-m2_slow_bad))[1]
  ...
  sd = standdevs, geom_mean = geometric_means, strat_A = female_strats)
